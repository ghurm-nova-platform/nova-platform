...
class ApprovalGateServiceTest {
    ...
    @Test
    void runWithFullPipelineRequiresHumanApproval() throws Exception {
        UUID taskId = seedFullPipeline("approval-happy-");
        mockMvc.perform(post("/api/approval-gate/run")
                        .header("Authorization", "Bearer " + accessToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"taskId\":\"" + taskId + "\"}"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.decision").value(ApprovalDecisionValue.REQUIRES_HUMAN_APPROVAL.name()))
                .andExpect(jsonPath("$.eligibleForMerge").value(false));
    }

    @Test
    void runWithoutEvidenceReturnsBlocked() throws Exception {
        UUID taskId = PullRequestTestFixture.createTask(
                mockMvc, objectMapper, accessToken, "approval-empty-" + UUID.randomUUID());
        mockMvc.perform(post("/api/approval-gate/run")
                        .header("Authorization", "Bearer " + accessToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"taskId\":\"" + taskId + "\"}"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.decision").value(ApprovalDecisionValue.BLOCKED.name()));
    }

    @Test
    void approveAfterHumanRequiredCanReachApproved() throws Exception {
        ApprovalTestFixture.allowAuthorApproval(policyRepository);
        UUID taskId = seedFullPipeline("approval-approve-");
        mockMvc.perform(post("/api/approval-gate/run")
                        .header("Authorization", "Bearer " + accessToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"taskId\":\"" + taskId + "\"}"))
        ...
        mockMvc.perform(post("/api/approval-gate/" + taskId + "/approve")
                ...
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.decision").value(ApprovalDecisionValue.APPROVED.name()))
                .andExpect(jsonPath("$.eligibleForMerge").value(true));
    }

    private UUID seedFullPipeline(String prefix) throws Exception {
        return ApprovalTestFixture.seedTaskWithFullPipeline(
                ...
                configRepository,
                taskRepository,
                Path.of(System.getProperty("java.io.tmpdir"), "nova-approval-service-tests"),
                prefix);
    }
}
