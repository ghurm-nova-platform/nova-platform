...
class ApprovalFailurePersistenceTest {
    ...
    @Autowired
    private ApprovalGateOperationRepository operationRepository;
    ...
    @Test
    void blockedRunPersistsOperationWhenEvidenceIncomplete() throws Exception {
        UUID taskId = PullRequestTestFixture.createTask(
                mockMvc, objectMapper, accessToken, "approval-fail-" + UUID.randomUUID());
        ...
        assertThat(operationRepository.findByTaskIdAndOrganizationIdOrderByCreatedAtDesc(
                        taskId, ApprovalTestFixture.ORG_ID))
                .isNotEmpty();
    }

    @Test
    void successfulEvaluationPersistsDecision() throws Exception {
        UUID taskId = ApprovalTestFixture.seedTaskWithFullPipeline(
                ...
                configRepository,
                taskRepository,
                Path.of(System.getProperty("java.io.tmpdir"), "nova-approval-fail-tests"),
                "approval-fail-ok-");
        ...
        assertThat(operationRepository.findFirstByTaskIdAndOrganizationIdOrderByCreatedAtDesc(
                        taskId, ApprovalTestFixture.ORG_ID)
                .orElseThrow()
                .getDecision())
                .isEqualTo(ApprovalDecisionValue.REQUIRES_HUMAN_APPROVAL);
    }
}
