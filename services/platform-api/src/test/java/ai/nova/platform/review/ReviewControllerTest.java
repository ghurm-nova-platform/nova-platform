...
class ReviewControllerTest {
    ...
    @Test
    void runAndGetLatestReview() throws Exception {
        UUID taskId = createTaskWithArtifact("review-api-" + UUID.randomUUID());

        mockMvc.perform(post("/api/review/run")
                ...
                .andExpect(jsonPath("$.severityCounts.MEDIUM").value(1));

        MvcResult getResult = mockMvc.perform(get("/api/review/" + taskId)
                        .header("Authorization", "Bearer " + accessToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.summary").value("Overall code quality is good."))
                .andReturn();
        JsonNode body = objectMapper.readTree(getResult.getResponse().getContentAsString());
        assertThat(body.get("reviewedArtifacts").isArray()).isTrue();
        assertThat(body.get("reviewedArtifacts").size()).isEqualTo(1);
    }
    ...
    void runUnknownTaskReturnsNotFound() throws Exception {
        mockMvc.perform(post("/api/review/run")
                ...
                .andExpect(jsonPath("$.code").value("REVIEW_TASK_NOT_FOUND"));
    }
