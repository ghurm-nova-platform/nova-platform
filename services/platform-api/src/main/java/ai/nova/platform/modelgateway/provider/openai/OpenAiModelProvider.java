...
public class OpenAiModelProvider implements AiModelProvider {
    ...
    public static final String ADAPTER_KEY = "OPENAI";
    ...
    public ProviderInvokeResult invoke(ProviderInvokeRequest request) throws ProviderException {
        ...
        URI baseUri = restClientFactory.resolveOpenAiBaseUrl();
        ...
        try {
            JsonNode response = client.post()
                    ...
                    .header("Authorization", "Bearer " + request.credentialSecret())
                    .body(body)
                    .retrieve()
                    .body(JsonNode.class);
    ...
    ProviderInvokeResult toResult(JsonNode response, long startNanos) throws ProviderException {
        if (response == null) {
    ...
    private static void validateEndpoint(ProviderInvokeRequest request) throws ProviderException {
        if (request.endpoint() != null
                && request.endpoint().endpointProfile() != null
                && request.endpoint().endpointProfile() != EndpointProfile.OPENAI_PUBLIC) {
            throw new ProviderException(
                    "ENDPOINT_PROFILE_INVALID",
                    ProviderFailureKind.PERMANENT,
                    "OPENAI requires OPENAI_PUBLIC endpoint profile");
        }
    }
}
