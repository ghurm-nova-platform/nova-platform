...
- Multi-agent orchestration foundation, Planner Agent (plan → draft run import), Coding Agent (task → generated artifacts), Review Agent (artifacts → findings), Testing Agent (artifacts → test plans; no execution), and Patch Agent (approved artifacts → Unified Diff; no git apply)
