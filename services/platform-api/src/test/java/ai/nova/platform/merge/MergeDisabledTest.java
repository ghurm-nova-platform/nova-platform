...
@SpringBootTest(properties = "nova.merge.enabled=false")
@AutoConfigureMockMvc
class MergeDisabledTest {
    ...
    @Test
    void disabledMergeAgentReturnsServiceUnavailable() throws Exception {
        ...
        UUID taskId = MergeTestFixture.seedApprovedTask(
                ...
                configRepository,
                taskRepository,
                Path.of(System.getProperty("java.io.tmpdir"), "nova-merge-disabled-tests"),
                "merge-disabled-");
        ...
        mockMvc.perform(post("/api/merge/run")
                ...
                .andExpect(jsonPath("$.code").value("MERGE_DISABLED"));
    }
}
