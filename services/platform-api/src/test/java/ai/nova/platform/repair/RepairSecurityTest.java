...
class RepairSecurityTest {
    ...
    @Test
    void rejectsRunWithoutRepairRunPermission() throws Exception {
        String limitedToken = jwtService.createAccessToken(new AuthenticatedUser(
                RepairTestFixture.USER_ID,
                RepairTestFixture.ORG_ID,
                "viewer@nova.local",
                "Viewer",
                List.of("USER"),
                List.of("REPAIR_READ"),
    ...
    @Test
    void rejectsGetLatestWithoutRepairReadPermission() throws Exception {
        String limitedToken = jwtService.createAccessToken(new AuthenticatedUser(
                RepairTestFixture.USER_ID,
                RepairTestFixture.ORG_ID,
                "runner@nova.local",
        ...
        mockMvc.perform(get("/api/repair/" + UUID.randomUUID()).header("Authorization", "Bearer " + limitedToken))
                .andExpect(status().isForbidden())
                .andExpect(jsonPath("$.code").value("FORBIDDEN"));
    }
}
