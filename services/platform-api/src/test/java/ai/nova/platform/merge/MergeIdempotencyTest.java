...
class MergeIdempotencyTest {
    ...
    @Test
    void secondRunReturnsSameSucceededOperation() throws Exception {
        UUID taskId = MergeTestFixture.seedApprovedTask(
                ...
                configRepository,
                taskRepository,
                Path.of(System.getProperty("java.io.tmpdir"), "nova-merge-idempotency-tests"),
                "merge-idem-");

        MvcResult first = mockMvc.perform(post("/api/merge/run")
        ...
        MvcResult second = mockMvc.perform(post("/api/merge/run")
                        .header("Authorization", "Bearer " + accessToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"taskId\":\"" + taskId + "\"}"))
                .andExpect(status().isOk())
        ...
        MergeOperation firstOp = objectMapper.readValue(first.getResponse().getContentAsString(), MergeOperation.class);
        MergeOperation secondOp = objectMapper.readValue(second.getResponse().getContentAsString(), MergeOperation.class);
        assertThat(secondOp.id()).isEqualTo(firstOp.id());
        assertThat(secondOp.result().mergedCommit()).isEqualTo(firstOp.result().mergedCommit());
    }
}
