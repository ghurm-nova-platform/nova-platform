...
- Enterprise Identity — unified auth entry wrapping JWT; SAML/OIDC/OAuth2/LDAP/AD providers, sessions, login history, MFA (TOTP), optional SCIM, password policies (V58)
- Local LLM Runtime — gateway-only local inference (Ollama / llama.cpp / vLLM / deterministic), model lifecycle, conversations, prompts; no direct provider calls from other modules (V59)
...
- Multi-Agent Collaboration — session-based coordination of existing agents with shared context, immutable messages, parallel groups, conflict detection, and append-only timeline (V55)
